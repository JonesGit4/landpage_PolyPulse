    ...
    <footer className="border-t border-white/5 pt-20 pb-10">
      <div className="max-w-6xl mx-auto px-6 sm:px-8 lg:px-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-14">
          ...
          {/* Product */}
          <div>
            <h4 className="text-white font-heading font-semibold text-sm mb-5 uppercase tracking-wider text-xs">
              {f.product}
            </h4>
          ...
          <div>
            ...
            <div className="flex gap-2 mb-6">
              <input
                ...
                className="flex-1 bg-dark-800 border border-white/[0.08] rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-primary/40 transition-colors font-light"
              />
              <button className="px-4 py-2.5 bg-primary text-dark-900 font-bold text-sm rounded-lg hover:bg-primary/80 hover:shadow-[0_0_15px_rgba(0,212,255,0.2)] transition-all">
                {f.subscribe}
              </button>
            </div>
            {/* Social icons */}
            <div className="flex gap-2.5">
              {[
                ...
                <a key={s.name} href="#" className="w-10 h-10 rounded-lg bg-dark-800 border border-white/[0.06] flex items-center justify-center hover:border-primary/40 hover:bg-primary/[0.06] transition-all duration-300" title={s.name}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" className="text-gray-500 hover:text-primary transition-colors"><path d={s.svg} /></svg>
                </a>
              ))}
            </div>
          </div>
