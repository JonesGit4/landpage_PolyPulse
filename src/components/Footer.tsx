    ...
    <footer className="border-t border-white/5 pt-20 pb-10">
      <Container>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-14">
          ...
          <div>
            <h4 className="text-white font-heading font-semibold mb-5 uppercase tracking-wider text-xs">{f.product}</h4>
            <ul className="space-y-3">
              {['Scalper Bot', 'Swing Bot', 'Alert Bot', 'VIP Signals'].map((item) => (
                <li key={item}><a href="#bots" className="text-sm text-gray-500 hover:text-primary transition-colors font-light">{item}</a></li>
              ))}
            </ul>
          </div>
          <div>
            <h4 className="text-white font-heading font-semibold mb-5 uppercase tracking-wider text-xs">{f.resources}</h4>
            <ul className="space-y-3">
              {[f.docs, f.blog, f.tutorials, 'FAQ'].map((item, i) => (
                <li key={i}><a href="#" className="text-sm text-gray-500 hover:text-primary transition-colors font-light">{item}</a></li>
              ))}
            </ul>
          </div>
          <div>
            <h4 className="text-white font-heading font-semibold mb-5 uppercase tracking-wider text-xs">{f.newsletter}</h4>
            <div className="flex gap-2 mb-6">
              <input type="email" placeholder={f.emailPlaceholder} className="flex-1 bg-dark-800 border border-white/[0.08] rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-primary/40 transition-colors font-light" />
              <button className="px-4 py-2.5 bg-primary text-dark-900 font-bold text-sm rounded-lg hover:bg-primary/80 transition-all">{f.subscribe}</button>
            </div>
            <div className="flex gap-2.5">
              {[
                ...
                <a key={s.name} href="#" className="w-10 h-10 rounded-lg bg-dark-800 border border-white/[0.06] flex items-center justify-center hover:border-primary/40 hover:bg-primary/[0.06] transition-all duration-300" title={s.name}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" className="text-gray-500"><path d={s.svg} /></svg>
                </a>
              ))}
            </div>
          </div>
        ...
        <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6">
            {[f.terms, f.privacy, f.disclaimer].map((item, i) => (<a key={i} href="#" className="text-xs text-gray-600 hover:text-gray-400 transition-colors font-light">{item}</a>))}
          </div>
        </div>
