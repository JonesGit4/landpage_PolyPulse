    ...
    <section id="bots" className="py-28 relative">
      ...
      <Container>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {bots.map((bot, i) => (
            <div key={i} className={`group card-base card-shine p-6 cursor-pointer ${bot.glow}`}>
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-5">
                  <span className="text-3xl">{bot.icon}</span>
                  <span className={`text-[11px] font-bold px-3 py-1 rounded-full bg-gradient-to-r ${bot.gradient} text-dark-900 uppercase tracking-wider`}>{bot.badge}</span>
                </div>
